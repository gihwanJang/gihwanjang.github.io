...
## [시연] Constructor Injection

`git checkout -f feature/spring_constructor_injection`
...
## [실습] Constructor Injection (5분)

- HighSchoolStudent, UniversityStudent 을 생성자 주입으로 초기화해본다.
    

`git checkout -f feature/spring_constructor_injection_training`

## [시연] Setter Injection

`git checkout -f feature/spring_setter_injection`
...
## [실습] Setter Injection (5분)

- HighSchoolStudent, UniversityStudent 을 세터 주입으로 초기화해본다.
    

`git checkout -f feature/spring_setter_injection_training`

## [시연] Field Injection

`git checkout -f feature/spring_field_injection`
...
## [실습] Field Injection (5분)

- HighSchoolStudent, UniversityStudent 을 필드 주입으로 초기화해본다.
    

`git checkout -f feature/spring_field_injection_training`

## [시연] 주입할 Bean 을 더 명확하게 하고 싶다면? (@Primary, @Qualifier)

### @Bean 사용

`git checkout -f feature/spring_qualifier_injection`
...
`git checkout -f feature/spring_qualifier_injection2`

## [실습] 두레이 메신저에 훅 보내기 (20분)

- DoorayWebHookSender 을 빈으로 만들어서 두레이 훅 메시지를 보낸다.
    
- AppStartupRunner 를 이용한다.
    

`git checkout -f feature/spring_di_training`
